...
public class CreateEmployeePageController implements EmployeeNavigators {
        ...
        public CreateEmployeePageController(SessionFactory sessionFactory) {
        ...
        void saveButtonClicked(ActionEvent event) {
                ...
                String csvFilePath = "C:\\Users\\Abdallah\\Desktop\\Hr-Pulse_SergeyDir.employee_data.csv"; // Set your desired file path
                saveToCSV(employee, csvFilePath);


                // Update the database
                try (Session session = sessionFactory.openSession()) {
                        session.beginTransaction();
                        session.save(employee); // Save the newly created Employee
                        session.getTransaction().commit();
                } catch (Exception e) {
                        showErrorDialog("שגיאה בהכנסת המידע ל DATABASE");
                        return;
                }
